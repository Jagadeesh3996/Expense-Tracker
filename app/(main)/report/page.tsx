...
import { CreditCard, Tag, TrendingDown, TrendingUp, Wallet } from "lucide-react";
...
export default async function Page() {
  ...
  const totalMonthlyIncome = incomeData?.reduce((acc, curr) => acc + curr.amount, 0) || 0;
  const totalMonthlyBalance = totalMonthlyIncome - totalMonthlyExpense;
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
      <div className="grid auto-rows-min gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        ...
        <div
          className="group relative flex flex-col justify-between overflow-hidden rounded-xl bg-white p-5 shadow-sm transition-all hover:shadow-md dark:bg-muted/50 dark:shadow-none h-32 border border-transparent hover:border-indigo-100 dark:hover:border-indigo-900/50"
        >
          ...
          <div className="relative z-10 flex items-center justify-between">
            <h3 className="text-sm font-medium text-muted-foreground">This Month Balance</h3>
            <div className="rounded-full bg-indigo-50 p-2 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-400">
              <Wallet className="h-4 w-4" />
            </div>
          </div>

          <div className="relative z-10">
            <div className={`text-2xl font-bold ${totalMonthlyBalance >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
              ₹{totalMonthlyBalance.toLocaleString(undefined, { minimumFractionDigits: 2 })}
            </div>
            <p className="text-xs text-muted-foreground mt-0.5">
